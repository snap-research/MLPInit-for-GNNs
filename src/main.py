...
def arg_parse():
    ...
    parser.add_argument('--type_model', type=str, default="GraphSAGE", choices=['GraphSAGE', 'ClusterGCN', 'GraphSAINT', "GAT", "GCN"])
    ...
    parser.add_argument("--epochs", type=int, default=50, help="number of training the one shot model")
    ...
    parser.add_argument("--batch_size", type=int, default=1000, help="batch size depending on methods, " "need to provide fair batch for different approaches")
    ...
    ### added 
    parser.add_argument('--train_gnn_type', type=str, default='gnn', choices=['gnn', 'mlp'])
    parser.add_argument('--gnn_type', type=str, default='gnn', choices=['gnn', 'mlp'])
...
if __name__ == "__main__":
    ...
    # model training
    if os.path.exists( f'./{args.dataset}_{args.type_model}_{args.train_gnn_type}.pt') and args.use_checkpoint == True: # load PeerMLP
        trnr = trainer(args)
        trnr.test_gnn_mlp()

    else:
        args.gnn_type = args.train_gnn_type 
        trnr = trainer(args)
